	...
	return (
		<section className="h-[calc(80vh)] flex flex-col items-center justify-center">
			<div>
				<div>
					...
					<SlideFade delay={0.1}>
						<Link href="/games/about-me">
							<h1 className="text-23 leading-[80%] w-fit max-lg:text-19 max-sm:text-13 max-[400px]:text-9 font-extrabold uppercase outline outline-transparent transition-all hocus:outline-blue-400">
								{LANDING_TEXT.name}
							</h1>
						</Link>
					</SlideFade>
				...
				<SlideFade delay={0.2}>
					<Markdown className="text-muted-foreground text-11 font-medium leading-[95%] pt-4 w-3/4 max-lg:text-9 max-md:w-full max-sm:text-7 max-[400px]:text-5 mb-10">
					...
					{LANDING_BULLET_POINTS.map((points, key) => (
						<li key={key}>
							<SlideFade delay={0.3 + key * 0.1} className="flex items-center gap-4">
								...
								<Markdown className="text-9 text-muted-foreground max-lg:text-7 max-sm:text-5 max-[400px]:text-4 leading-[95%]">
									{points.text}
								</Markdown>
							</SlideFade>
						</li>
